<?php include("template_base/header.php"); ?>
    <main>
      <div class="colab">
        <div class="colab--top">
          <p class="title--72">CO-Lab</p>
          <p class="title--36 title--36-bold">
            We are open to the generation of collaborative content, either at
            our four locations or anywhere in the world. Our purpose is not to
            identify ourselves as a company, but as a group willing to create
            content that generates change and/or solves problems.<br /><br />The
            collaborative work we do with different production companies,
            artists and/or directors allows us to share and create better
            results. We feel ready to attend to any proposal and work it in the
            best way as we have been doing during these years.
          </p>
        </div>
        <button class="colab--scroll-bottom">
          <img src="/static/media/icon-scroll-down.bbf41f5a.svg" alt="" />
        </button>
        <div class="colab--bottom">
          <div class="colab--bottom--img1"></div>
          <div class="colab--bottom--img2"></div>
          <div class="colab--bottom--img3"></div>
          <div class="colab--bottom--img4"></div>
        </div>
      </div>
    </main>
<?php include("template_base/footer.php"); ?>
